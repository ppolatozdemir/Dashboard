...
async function exportUnsprintedExcel() {
    ...
    const btn = document.getElementById('exportUnsprintedExcelBtn');
    const oldText = btn ? btn.textContent : null;
    if (btn) { btn.disabled = true; btn.textContent = '? Haz?rlan?yor...'; }
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(err.error || 'Excel olu?turulamad?');
        }
...
// Her iki taraftaki sprint listelerini y?kle
async function loadUnsprintedSprints() {
    ...
    olkaSelect.innerHTML = '<option value="">Sprintler y?kleniyor...</option>';
    hebiarSelect.innerHTML = '<option value="">Sprintler y?kleniyor...</option>';
    
    // Olka sprintleri
    try {
        const res = await fetch('/api/unsprinted/olka-sprints');
        if (!res.ok) throw new Error((await res.json()).error || 'Olka sprintleri al?namad?');
        const sprints = await res.json();
        olkaSelect.innerHTML = '<option value="">Sprint se?in...</option>' + sprints.map(s =>
    ...
    try {
        ...
        if (!res.ok) throw new Error((await res.json()).error || 'Hebiar sprintleri al?namad?');
        const sprints = await res.json();
        hebiarSelect.innerHTML = '<option value="">Sprint se?in...</option>' + sprints.map(s =>
            `<option value="${s.id}"${s.state === 'active' ? ' selected' : ''}>${escapeHtml(s.name)}${s.state === 'active' ? ' (aktif)' : ''}</option>`
        ).join('');
    } catch (error) {
...
async function loadUnsprintedReport() {
    ...
    if (!olkaSprintId || !hebiarSprintId) {
        alert('L?tfen hem Olka hem de Hebiar sprintini se?in.');
        return;
    }
    ...
    container.innerHTML = `
        <div class="loading" style="height: 150px;">
            ...
            <span>Tasklar kar??la?t?r?l?yor...</span>
        </div>
    `;
    ...
    try {
        ...
        if (!res.ok) throw new Error((await res.json()).error || 'Rapor al?namad?');
...
function renderUnsprintedReport(data) {
    ...
    infoBar.innerHTML = `
        ...
        <span style="background: rgba(231,76,60,0.35); padding: 5px 12px; border-radius: 6px; font-size: 13px;">?? Sprinte Al?nmayan: <strong>${data.count}</strong></span>
        ${!data.cllinkFieldFound ? `<span style="background: rgba(243,156,18,0.35); padding: 5px 12px; border-radius: 6px; font-size: 13px;">?? CLLINK alan? bulunamad?</span>` : ''}
    `;
    ...
    if (rows.length === 0) {
        container.innerHTML = `
            <div style="text-align: center; padding: 30px; color: var(--text-secondary);">
                ?? Se?ilen Olka sprintindeki t?m tasklar Hebiar sprintine al?nm??.
            </div>
        `;
        return;
    }
        ...
        <table class="daily-report-table" style="width: 100%;">
            <thead>
                <tr>
                    ...
                    <th style="text-align: left;">Task Ad?</th>
                    <th style="text-align: left;">Atanan Ki?i</th>
                    <th style="text-align: left;">Reporter</th>
                    <th style="text-align: left;">Stat?s?</th>
                    <th style="text-align: left;">?ncelik Seviyesi</th>
                </tr>
            </thead>
            <tbody>
    `;
    ...
    rows.forEach(r => {
        ...
        tableHTML += `
            <tr>
                ...
                <td>${r.assignee ? escapeHtml(r.assignee) : '<span style="color: var(--text-muted);">Atanmam??</span>'}</td>
